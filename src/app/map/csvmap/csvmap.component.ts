...
export class CsvmapComponent implements OnInit {
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    var welllistarry: any = this.Wells;



    //  wellList.push(this.Wells)

    localStorage.setItem('welllist', JSON.stringify(welllistarry));

    //this.Wells = JSON.parse(localStorage.getItem('welllist'))
  ...
  t = loadModules(['esri', 'esri/map', "esri/geometry/Polygon", "esri/geometry/webMercatorUtils", "esri/graphic", "esri/symbols/Font", "esri/symbols/TextSymbol", "esri/SpatialReference", "esri/geometry/Polyline", "esri/toolbars/draw", "esri/InfoTemplate", "esri/geometry/Point", "esri/layers/GraphicsLayer", "esri/dijit/HomeButton", "dojo/domReady!"], options)



    .then(([esri, Map, Polygon, webMercatorUtils, Graphic, Font, TextSymbol, SpatialReference, Polyline, Draw, InfoTemplate, Point, GraphicsLayer, HomeButton]) => {
      ...
      clusterGraphicLayer.on("click", function (evt) {
        ...
        if (document.getElementById("map_container").style.cursor == "default")
        ...
        // li.setAttribute('id', (Number(selectedWells) + 1) + "-" + evt.graphic.attributes["uwi"]);
        ...
        // li.setAttribute('dropzone', "move");



        // li.appendChild(document.createTextNode((Number(selectedWells) + 1) + "-" + evt.graphic.attributes["uwi"]));
      ...
      function addWellToSelection(uwi, WellGeomery) {
        ...
        textSymbol.setHaloColor(new esri.Color([0, 255, 0]));
